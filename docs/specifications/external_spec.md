# 定数管理システム 外部仕様書

**版番:** 1.2  
**作成日:** 2026年1月21日  
**最終更新:** 2026年1月21日  
**対象システム:** 車載ソフトウェア定数管理ツール

---

## 1. システム概要

### 1.1 目的
本システムは、車載ソフトウェアの定数をGUIで一元管理し、CSVファイルを用いた定数データの入出力を可能にするデスクトップアプリケーションです。開発メンバー間での定数値の共有、外部編集、バージョン管理を効率化します。

### 1.2 システム構成
- **GUI 管理画面**: Windowsフォームベースの定数管理インターフェース（メニューバー、グリッド表示）
- **CSV入出力機能**: 定数データのエクスポート・インポート
- **ファイル保存**: 定数データの永続化
- **エラーハンドリング**: 不正なデータ形式や型不一致への対応
- **モーダルダイアログ**: 定数の追加・編集時に専用ダイアログを表示

### 1.3 主なユーザー
- 車載ソフトウェア開発エンジニア
- システムテスト担当者
- プロジェクトマネージャー

---

## 2. データ構造

### 2.1 定数アイテム (Constant Item)

定数管理システムで扱う最小単位のデータ構造を以下に定義します：

| 項目名 | 型 | 必須 | 制限・備考 |
|--------|-----|------|----------|
| **PhysicalName** | 文字列 | ◎ | システムの主キーであり、C言語コードの変数名。英大文字とアンダースコアのみ。最大32文字。新規作成時は入力可、既存編集時は読み取り専用。 |
| **LogicalName** | 文字列 | ◎ | 定数の日本語名（論理名）。最大64文字。 |
| **Value** | 数値/文字列 | ◎ | 設定値。数値またはテキスト。 |
| **Unit** | 文字列 | ○ | 単位情報（例："℃", "km/h", "V"）。最大16文字。省略可能。 |
| **Description** | 文字列 | ○ | 定数の説明・用途。最大256文字。省略可能。 |

**注記:**
- PhysicalName は CSVインポート時の主キーとして機能
- Value は数値、小数、負数、または文字列を受け付ける
- 日本語を含むテキストは UTF-8 で処理
- PhysicalName（定数名/物理名）は変数名として C言語コードで使用される

---

## 3. 機能要件

### 3.1 画面レイアウト

#### 3.1.1 全体構成

```
┌──────────────────────────────────────┐
│ 定数管理システム                     │
├──────────────────────────────────────┤
│ ファイル(F)  編集(E)                 │  ← メニューバー（MenuStrip）
├──────────────────────────────────────┤
│                                      │
│  定数名 | 日本語名 | 値 | 単位 | 説明│  ← グリッド表示
│  ──────────────────────────────────│  ← データ行（スクロール可能）
│                                      │
│                                      │
│                                      │
│                                      │
│                                      │
├──────────────────────────────────────┤
│ 全 {n} 件 | 選択 {m} 件              │  ← ステータスバー
└──────────────────────────────────────┘
```

#### 3.1.2 コントロール仕様

**メニューバー（MenuStrip）:**
- **[ファイル(F)]**
  - 読込(O)... - CSVファイルをインポート。既存データを上書きまたはマージするかを選択
  - 保存(S) - 現在のデータセットをCSVファイルにエクスポート（全行）
  - CSVエクスポート(E)... - 選択行のみをCSVファイルにエクスポート
- **[編集(E)]**
  - 新規追加(N)... - 新規定数追加ダイアログを表示
  - 編集(D)... - 選択行の定数編集ダイアログを表示
  - 削除(L) - 選択行を削除

**グリッド表示:**
- **DataGridView** コントロール
- 6列表示: チェックボックス, 定数名（物理名）, 日本語名（論理名）, 値, 単位, 説明
- 左端にチェックボックス列を配置
- 複数行選択対応（チェックボックスで選択）
- ソート機能（カラムヘッダークリックでソート）
- 縦横スクロール対応

**選択状態の表示:**
- 選択行のセルを黄色でハッチング処理（斜線パターン）
- ハッチング色: RGB(255, 255, 0)（黄色）
- ハッチングスタイル: 斜線パターン（ForwardDiagonal）
- チェックボックスの状態と視覚的なハッチングで選択状態を明確化

---

### 3.2 CSVエクスポート機能

#### 3.2.1 仕様

- **対象データ**: ユーザーがチェックボックスで選択した行のみをCSV形式で出力
- **ヘッダー行**: 必ず1行目に各カラム名を含める
- **文字コード**: UTF-8（BOM付き必須）
- **改行コード**: CRLF（Windows標準）
- **ファイル形式**: `.csv`
- **デリミタ**: カンマ `,`

#### 3.2.2 出力例

```csv
PhysicalName,LogicalName,Value,Unit,Description
ENGINE_TEMP_MAX,エンジン最高温度,120,℃,エンジンの最大許容温度
SPEED_LIMIT,速度制限,100,km/h,法定速度制限
BATTERY_VOLTAGE,バッテリー電圧,12.0,V,正常動作時の基準電圧
```

#### 3.2.3 特殊文字の処理

- CSVデータ内にカンマやダブルクォートを含む場合は、ダブルクォートで囲む
- 改行を含む場合はダブルクォートで囲む

**例:**
```csv
PhysicalName,LogicalName,Value,Unit,Description
SENSOR_RANGE,センサー検出範囲,"0-100","%","正常範囲: 0～100"
```

---

### 3.3 CSVインポート機能

#### 3.3.1 仕様

- **ファイル形式**: `.csv` ファイル
- **文字コード**: UTF-8（BOM付き必須）、SHIFT_JIS にも対応
- **処理ロジック**:
  1. CSVファイルを読み込む
  2. ヘッダー行をスキップ
  3. 各データ行に対し:
     - **PhysicalName が既に存在** → 既存行を更新
     - **PhysicalName が新規** → 新規行として追加
- **マージモード**: 既存データと新規データをマージ（デフォルト動作）
- **置換モード**: 既存データをすべて削除して新データで置換

#### 3.3.2 インポート時の検証

| 検証項目 | 内容 | エラー時の動作 |
|---------|------|----------------|
| **ヘッダー行** | PhysicalName, LogicalName, Value, Unit, Description が存在することを確認 | エラー表示、インポート中止 |
| **必須項目** | PhysicalName, LogicalName, Value が空でないことを確認 | その行をスキップ、警告表示 |
| **PhysicalName 形式** | 英大文字とアンダースコアのみ、最大32文字 | エラー表示、その行をスキップ |
| **Value 型** | 数値または文字列を判定（自動判別） | そのまま受け入れ |
| **重複チェック** | 同一ファイル内での PhysicalName 重複を検出 | エラー表示、後の行を優先 |

---

### 3.4 エラー処理

#### 3.4.1 エラーメッセージ一覧

| エラーコード | メッセージ | 原因 | ユーザーアクション |
|------------|-----------|------|------------------|
| **E001** | "ファイルが見つかりません" | 指定ファイルが存在しない | ファイルパスを確認 |
| **E002** | "ファイルの読み込みに失敗しました" | ファイルロック、権限不足 | ファイルを閉じるか権限を確認 |
| **E003** | "不正なCSV形式です（ヘッダー行が不正）" | ヘッダー行が仕様と異なる | CSVファイル形式を確認 |
| **E004** | "エンコーディングが認識できません" | サポート外の文字コード | UTF-8 または SHIFT_JIS に変換 |
| **E005** | "データ行 {行番号}: PhysicalName が空です" | 必須項目が欠落 | その行の PhysicalName を入力 |
| **E006** | "データ行 {行番号}: LogicalName が空です" | 必須項目が欠落 | その行の LogicalName を入力 |
| **E007** | "データ行 {行番号}: Value が空です" | 必須項目が欠落 | その行の Value を入力 |
| **E008** | "データ行 {行番号}: PhysicalName の形式が不正です（英大文字とアンダースコアのみ）" | PhysicalName が仕様を逸脱 | 正しい形式に修正 |
| **E009** | "データ行 {行番号}: PhysicalName の長さが上限を超えています（最大32文字）" | PhysicalName が長すぎる | 32文字以内に短縮 |
| **E010** | "データ行 {行番号}: 重複する PhysicalName があります（{PhysicalName}）" | 同一ファイル内で PhysicalName が重複 | PhysicalName を修正 |
| **E011** | "保存に失敗しました（ディスク容量不足）" | ストレージ容量不足 | ストレージを確認・清掃 |
| **E012** | "ファイルへの書き込み権限がありません" | 権限不足 | ファイルの権限設定を確認 |
| **W001** | "警告: データ行 {行番号} は必須項目が欠落しているためスキップされました" | 不完全な行がある | 該当行を確認・修正 |
| **W002** | "警告: {n} 行のデータがインポートされました。{m} 行はスキップされました" | 一部の行がスキップされた | 結果を確認 |

#### 3.4.2 エラーダイアログ例

```
[エラー]
───────────────────────────────
データ行 5: PhysicalName の形式が不正です
（英大文字とアンダースコアのみ）

PhysicalName: "Engine-Temp" は無効な文字を含みます

[OK]
───────────────────────────────
```

---

## 4. 画面UI構成案

### 4.1 メイン画面詳細

#### 4.1.1 メニューバー（MenuStrip）
- **配置**: 画面最上部
- **メニュー構成**:
  - **[ファイル(F)]**
    - 読込(O)... - CSVファイルを開く（ファイル選択ダイアログ）
    - 保存(S) - 全データをCSVエクスポート
    - CSVエクスポート(E)... - 選択行のみCSVエクスポート
  - **[編集(E)]**
    - 新規追加(N)... - 定数編集ダイアログを新規モードで表示
    - 編集(D)... - 定数編集ダイアログを編集モードで表示
    - 削除(L) - 選択行を削除

#### 4.1.2 グリッド表示
- **位置**: 画面中央、メニューバー下からステータスバーの上まで
- **サイズ**: ウィンドウ幅に合わせて自動調整
- **カラム幅**:
  - チェックボックス: 40px（固定）
  - 定数名（物理名）: 120px（固定）
  - 日本語名（論理名）: 150px（自動調整可）
  - 値: 100px（自動調整可）
  - 単位: 80px（自動調整可）
  - 説明: 残り全幅
- **行高**: 24px
- **選択モード**: 複数行選択対応（チェックボックスで選択）
- **ソート**: カラムヘッダークリックでソート可能（昇順/降順）
- **ハイライト**: 選択行は黄色でハッチング

#### 4.1.3 ステータスバー
- **配置**: 画面下部
- **表示内容**:
  - 総行数: "全 {n} 件"
  - 選択行数: "選択 {m} 件"
  - 最終更新日時: "最終更新: YYYY-MM-DD HH:MM:SS"

---

### 4.2 ダイアログ設計

#### 4.2.1 [ファイル] - [読込] - CSVファイル選択ダイアログ
```
ファイルを開く
─────────────────────────────
場所: [C:\Users\...\]
ファイル名: [_________________]
ファイルの種類: [CSV ファイル (*.csv)]

[ マージモード ○] [ 置換モード ○]

[開く] [キャンセル]
─────────────────────────────
```

#### 4.2.2 [ファイル] - [保存] / [CSVエクスポート] - ファイル保存ダイアログ
```
名前を付けて保存
─────────────────────────────
場所: [C:\Users\...\]
ファイル名: [constants_YYYYMMDD.csv]
ファイルの種類: [CSV ファイル (*.csv)]

[保存] [キャンセル]
─────────────────────────────
```

#### 4.2.3 エラー確認ダイアログ
```
[エラー]
─────────────────────────────
{エラーメッセージ}

詳細情報:
{エラー詳細（複数行可）}

[OK] [詳細を表示] [ログに保存]
─────────────────────────────
```

---

### 4.3 定数編集ダイアログ

#### 4.3.1 概要
定数の新規追加または既存データの編集を行うモーダルダイアログです。新規追加モードと既存編集モードの2つの状態があります。

#### 4.3.2 レイアウト

```
┌─────────────────────────────────┐
│ 定数を編集                       │
├─────────────────────────────────┤
│                                 │
│ 定数名(物理名):                  │
│ [_________________] ※編集時読取専用 │
│                                 │
│ 日本語名(論理名):                │
│ [_________________]              │
│                                 │
│ 値:                             │
│ [_________________]              │
│                                 │
│ 単位:                           │
│ [__________]                    │
│                                 │
│ 説明:                           │
│ [_________________________]      │
│ [_________________________]      │
│ [_________________________]      │
│                                 │
│              [OK] [キャンセル]   │
│                                 │
└─────────────────────────────────┘
```

#### 4.3.3 フィールド仕様

| フィールド名 | 入力形式 | 最大文字数 | 新規モード | 編集モード | 備考 |
|------------|---------|----------|----------|----------|------|
| **定数名(物理名)** | テキストボックス | 32 | 入力可 | 読み取り専用 | 英大文字・アンダースコアのみ。編集モードではフォーカス不可、グレーアウト表示 |
| **日本語名(論理名)** | テキストボックス | 64 | 入力可 | 入力可 | 日本語対応 |
| **値** | テキストボックス | 256 | 入力可 | 入力可 | 数値または文字列 |
| **単位** | テキストボックス | 16 | 入力可 | 入力可 | 省略可能 |
| **説明** | マルチラインテキストボックス | 256 | 入力可 | 入力可 | 複数行対応 |

#### 4.3.4 ボタン仕様

- **[OK]** - 入力値を検証し、グリッドに新規行を追加または選択行を更新して、ダイアログを閉じる
  - 検証エラーがある場合は、エラーメッセージを表示し、ダイアログを開いたまま
- **[キャンセル]** - 入力内容を破棄してダイアログを閉じる

#### 4.3.5 モード別の動作

**新規追加モード（新しい定数を追加）:**
- タイトル: "定数を編集"
- 定数名(物理名): 入力可能、フォーカスあり（最初のフィールド）
- 他フィールド: 空白
- [OK] 押下時: 新規行をグリッドの最後に追加

**既存編集モード（選択した行を編集）:**
- タイトル: "定数を編集"
- 定数名(物理名): 読み取り専用、グレーアウト、フォーカス不可
- 他フィールド: 選択行の値が自動入力される
- フォーカス: 日本語名(論理名)に設定
- [OK] 押下時: グリッド内の選択行を更新

---

### 4.4 終了確認ダイアログ

#### 4.4.1 概要
ウィンドウの「×」ボタン（閉じるボタン）が押下された際に、未保存の変更がある場合に表示されるダイアログです。

#### 4.4.2 レイアウト

```
┌──────────────────────────────┐
│ 定数管理システム             │
├──────────────────────────────┤
│                              │
│ 保存されていない変更がある   │
│ ファイルに保存しますか?      │
│                              │
│  [はい(保存して終了)]        │
│  [いいえ(保存せず終了)]      │
│  [キャンセル]                │
│                              │
└──────────────────────────────┘
```

#### 4.4.3 ボタン仕様

- **[はい(保存して終了)]** - 現在のデータセットをCSVファイルにエクスポート（全行）後、アプリケーションを終了
  - 保存に失敗した場合は、エラーメッセージを表示し、アプリケーションは終了しない
- **[いいえ(保存せず終了)]** - 変更を破棄してアプリケーションを終了
- **[キャンセル]** - ダイアログを閉じ、アプリケーションを開いたまま

---

## 5. ユースケースフロー

### 5.1 UC-001: 定数を修正してCSV出力する

**前提条件**: 定数管理システムが起動している

**主シーン**:

| ステップ | アクター | アクション | システム応答 |
|---------|---------|-----------|-----------|
| 1 | ユーザー | [ファイル] → [読込]をクリック | ファイル選択ダイアログが表示 |
| 2 | ユーザー | 既存のCSVファイルを選択し、[開く] | CSV ファイルを読み込みして検証 |
| 3 | システム | - | グリッドに読み込んだデータを表示。ステータスに行数を表示 |
| 4 | ユーザー | グリッド内で修正対象行のチェックボックスをチェック | 選択行が黄色でハッチング |
| 5 | ユーザー | [編集] → [編集]をクリック | 定数編集ダイアログ（編集モード）が開く |
| 6 | ユーザー | 日本語名や値などを修正し [OK]をクリック | グリッド内のデータが更新される、ダイアログが閉じる |
| 7 | ユーザー | 修正対象行（1～5行）を複数選択し、[ファイル] → [CSVエクスポート] | ファイル保存ダイアログが表示 |
| 8 | ユーザー | ファイル名を入力し [保存] | 選択行のみがCSVに出力される |
| 9 | システム | - | 「保存完了」メッセージ表示 |

**終了条件**: CSVファイルに修正データが出力される

---

### 5.2 UC-002: 新規定数を追加する

**前提条件**: 定数管理システムが起動している、グリッドにデータが表示されている

**主シーン**:

| ステップ | アクター | アクション | システム応答 |
|---------|---------|-----------|-----------|
| 1 | ユーザー | [編集] → [新規追加]をクリック | 定数編集ダイアログ（新規モード）が開く |
| 2 | ユーザー | 定数名(物理名)に "NEW_CONSTANT" を入力 | テキストボックスに入力される |
| 3 | ユーザー | 日本語名(論理名)に "新規定数" を入力 | テキストボックスに入力される |
| 4 | ユーザー | 値に "100" を入力 | テキストボックスに入力される |
| 5 | ユーザー | 単位に "℃" を入力 | テキストボックスに入力される |
| 6 | ユーザー | 説明に "説明" を入力 | テキストボックスに入力される |
| 7 | ユーザー | [OK]をクリック | 新規行がグリッド最後に追加される、ダイアログが閉じる |
| 8 | ユーザー | [ファイル] → [保存]をクリック | ファイル保存ダイアログ表示 |
| 9 | ユーザー | ファイルを保存 | "保存完了"メッセージ表示 |

**終了条件**: 新規定数がグリッドに追加され、CSVファイルに反映される

---

### 5.3 UC-003: CSVファイルをインポート・マージする

**前提条件**: 定数管理システムが起動している、既存データが存在

**主シーン**:

| ステップ | アクター | アクション | システム応答 |
|---------|---------|-----------|-----------|
| 1 | ユーザー | [ファイル] → [読込]をクリック | ファイル選択ダイアログが表示 |
| 2 | ユーザー | 外部編集後のCSVファイルを選択 | ファイル名がダイアログに表示 |
| 3 | ユーザー | マージモードを選択して [開く] | CSV ファイルを読み込んで検証 |
| 4 | システム | - | 既存 PhysicalName と新規 PhysicalName を比較処理 |
| 5 | システム | - | 既存 PhysicalName は値を更新、新規 PhysicalName は行を追加 |
| 6 | システム | - | グリッドを更新、完了メッセージ表示 |
| 7 | ユーザー | [ファイル] → [保存]をクリック | ファイル保存ダイアログ表示 |
| 8 | ユーザー | ファイルを保存 | マージされたデータが保存される |

**終了条件**: 既存データが更新され、新規データが追加されたCSVが生成される

---

### 5.4 UC-004: 定数を削除する

**前提条件**: 定数管理システムが起動している、グリッドにデータが表示されている

**主シーン**:

| ステップ | アクター | アクション | システム応答 |
|---------|---------|-----------|-----------|
| 1 | ユーザー | グリッド内で削除対象行のチェックボックスをチェック | 選択行が黄色でハッチング |
| 2 | ユーザー | [編集] → [削除]をクリック | 選択行がグリッドから削除される |
| 3 | ユーザー | [ファイル] → [保存]をクリック | ファイル保存ダイアログ表示 |
| 4 | ユーザー | ファイルを保存 | 削除済みデータが反映されたCSVが保存される |

**終了条件**: 定数がグリッドから削除され、CSVファイルに反映される

---

### 5.5 UC-005: アプリケーションを終了する（未保存の変更がある場合）

**前提条件**: 定数管理システムが起動している、未保存の変更がある

**主シーン**:

| ステップ | アクター | アクション | システム応答 |
|---------|---------|-----------|-----------|
| 1 | ユーザー | グリッドデータを修正 | グリッド内のデータが更新される |
| 2 | ユーザー | ウィンドウの「×」ボタンをクリック | 終了確認ダイアログが表示 |
| 3 | ユーザー | [はい(保存して終了)]をクリック | 現在のデータセットをCSVファイルにエクスポート |
| 4 | システム | - | ファイル保存ダイアログが表示 |
| 5 | ユーザー | ファイルを保存 | アプリケーションが終了 |

**終了条件**: 変更内容が保存され、アプリケーションが終了される

---

### 5.6 UC-006: エラーハンドリング - 不正なCSV形式

**前提条件**: 不正なフォーマットのCSVファイルが存在

**主シーン**:

| ステップ | アクター | アクション | システム応答 |
|---------|---------|-----------|-----------|
| 1 | ユーザー | [ファイル] → [読込]をクリック | ファイル選択ダイアログが表示 |
| 2 | ユーザー | 不正なCSVファイルを選択して [開く] | ファイル読み込み・検証処理開始 |
| 3 | システム | - | ヘッダー行を検証 → 不正を検出 |
| 4 | システム | - | エラーダイアログ表示: "E003: 不正なCSV形式です（ヘッダー行が不正）" |
| 5 | ユーザー | エラーダイアログで [OK] | ダイアログが閉じられ、グリッドは変更なし |

**終了条件**: エラーが表示され、既存データは保全される

---

## 6. ファイル仕様

### 6.1 CSVファイル形式仕様

#### 6.1.1 基本情報
- **拡張子**: `.csv`
- **文字コード**: UTF-8（BOM付き必須）、SHIFT_JIS にも対応
- **改行コード**: CRLF（Windows標準）
- **デリミタ**: カンマ `,`
- **テキスト囲み文字**: ダブルクォート `"`

#### 6.1.2 ファイル構造

```
行番号 | 内容
────────────────────────────────────────────────────
1      | PhysicalName,LogicalName,Value,Unit,Description
2      | CONST_001,定数名1,値1,単位1,説明1
3      | CONST_002,定数名2,値2,単位2,説明2
...    | ...
n      | CONST_nnn,定数名n,値n,単位n,説明n
```

#### 6.1.3 カラム仕様表

| カラム番号 | カラム名 | データ型 | 最大長 | 必須 | 備考 |
|-----------|---------|---------|--------|------|------|
| 1 | PhysicalName | String | 32 | ◎ | C言語変数名。英大文字・アンダースコアのみ |
| 2 | LogicalName | String | 64 | ◎ | 日本語対応 |
| 3 | Value | String | 256 | ◎ | 数値、小数、文字列対応 |
| 4 | Unit | String | 16 | ○ | 単位表示（省略可） |
| 5 | Description | String | 256 | ○ | 説明文（省略可） |

#### 6.1.4 データ型と表現例

**Value カラムの例**:
- 整数: `100`, `-50`, `0`
- 小数: `3.14`, `-2.5`, `0.001`
- 文字列: `"HIGH"`, `"ON"`, `"abc123"`
- 特殊: `""` （空文字列）

**Unit カラムの例**:
- 温度: `℃`, `°C`, `K`
- 速度: `km/h`, `m/s`, `mph`
- 電圧: `V`, `mV`
- パーセント: `%`
- その他: `rpm`, `Hz`, `kPa`

#### 6.1.5 テキスト囲み処理の例

**カンマを含む場合**:
```csv
PhysicalName,LogicalName,Value,Unit,Description
RANGE_DEF,検出範囲,"0,100","%",説明
```

**改行を含む場合**:
```csv
PhysicalName,LogicalName,Value,Unit,Description
SENSOR_NOTE,センサー説明,値,単位,"1行目
2行目"
```

**ダブルクォートを含む場合**:
```csv
PhysicalName,LogicalName,Value,Unit,Description
FORMAT_STR,フォーマット,"\"VALUE\": %d",unit,説明
```

---

### 6.2 ファイル出力例

#### 6.2.1 完全な出力例

```csv
PhysicalName,LogicalName,Value,Unit,Description
ENGINE_TEMP_MAX,エンジン最高温度,120,℃,エンジンの最大許容温度
ENGINE_TEMP_MIN,エンジン最低温度,0,℃,エンジンの最小許容温度
SPEED_LIMIT,速度制限,100,km/h,法定速度制限値
BATTERY_VOLTAGE_NOMINAL,バッテリー基準電圧,12.0,V,正常動作時の基準電圧値
SENSOR_RANGE,センサー検出範囲,"0-100",%,正常範囲を示す文字列
SYSTEM_MODE,システムモード,AUTO,,自動/手動選択モード
```

#### 6.2.2 選択行のみ出力例

ユーザーが 3 行のみを選択した場合：

```csv
PhysicalName,LogicalName,Value,Unit,Description
ENGINE_TEMP_MAX,エンジン最高温度,120,℃,エンジンの最大許容温度
SPEED_LIMIT,速度制限,100,km/h,法定速度制限値
BATTERY_VOLTAGE_NOMINAL,バッテリー基準電圧,12.0,V,正常動作時の基準電圧値
```

---

## 7. エラーメッセージ一覧

### 7.1 エラーメッセージ詳細定義

#### ファイルI/O エラー

| コード | メッセージ | 説明 | 対処方法 |
|--------|-----------|------|---------|
| **E001** | ファイルが見つかりません | 指定したファイルが存在しない | ファイルパスを確認してください |
| **E002** | ファイルの読み込みに失敗しました | ファイルがロック状態またはアクセス権限なし | ファイルが開いていないか確認し、アクセス権限を確認 |
| **E011** | 保存に失敗しました（ディスク容量不足） | ストレージ容量が不足 | ディスク容量を確認・解放してください |
| **E012** | ファイルへの書き込み権限がありません | ファイルまたはディレクトリへの書き込み権限がない | ファイルの権限設定を確認してください |

#### CSV形式エラー

| コード | メッセージ | 説明 | 対処方法 |
|--------|-----------|------|---------|
| **E003** | 不正なCSV形式です（ヘッダー行が不正） | ヘッダー行が仕様と異なる | CSVファイルのヘッダー行が「PhysicalName,LogicalName,Value,Unit,Description」か確認 |
| **E004** | エンコーディングが認識できません | サポート外の文字コード | ファイルを UTF-8 または SHIFT_JIS に変換 |

#### データバリデーション エラー

| コード | メッセージ | 説明 | 対処方法 |
|--------|-----------|------|---------|
| **E005** | データ行 {行番号}: PhysicalName が空です | PhysicalName フィールドが空欄 | {行番号} 行の PhysicalName を入力してください |
| **E006** | データ行 {行番号}: LogicalName が空です | LogicalName フィールドが空欄 | {行番号} 行の LogicalName を入力してください |
| **E007** | データ行 {行番号}: Value が空です | Value フィールドが空欄 | {行番号} 行の Value を入力してください |
| **E008** | データ行 {行番号}: PhysicalName の形式が不正です（英大文字とアンダースコアのみ） | PhysicalName に不正な文字が含まれている | PhysicalName は英大文字（A-Z）とアンダースコア（_）のみ使用可能 |
| **E009** | データ行 {行番号}: PhysicalName の長さが上限を超えています（最大32文字） | PhysicalName が32文字を超えている | PhysicalName を32文字以内に短縮してください |
| **E010** | データ行 {行番号}: 重複する PhysicalName があります（{PhysicalName}） | 同一ファイル内で同じ PhysicalName が存在 | {PhysicalName} を一意な PhysicalName に変更してください |

### 7.2 警告メッセージ一覧

| コード | メッセージ | 説明 | 対処方法 |
|--------|-----------|------|---------|
| **W001** | 警告: データ行 {行番号} は必須項目が欠落しているためスキップされました | 必須項目（PhysicalName, LogicalName, Value）が欠落している行 | その行を削除するか、欠落項目を入力してください |
| **W002** | 警告: {n} 行のデータがインポートされました。{m} 行はスキップされました | インポート処理で一部の行がスキップされた | ログを確認し、スキップされた行を修正してください |
| **W003** | 警告: Unit フィールドが超過しています（最大16文字）。{行番号} 行で切り詰められました | Unit が16文字を超えている | Unit を16文字以内に修正してください |
| **W004** | 警告: Description が超過しています（最大256文字）。{行番号} 行で切り詰められました | Description が256文字を超えている | Description を256文字以内に修正してください |

### 7.3 情報メッセージ一覧

| コード | メッセージ | 説明 |
|--------|-----------|------|
| **I001** | ファイルを開きました。{n} 件のデータを読み込みました | CSVインポート成功 |
| **I002** | ファイルに保存しました | CSVエクスポート成功 |
| **I003** | {n} 行のデータをエクスポートしました | 選択行エクスポート成功 |
| **I004** | データを追加しました | 新規定数追加成功 |
| **I005** | データを更新しました | 定数編集成功 |
| **I006** | データを削除しました | 定数削除成功 |

---

### 7.4 メッセージダイアログの表示方式

#### エラーダイアログ（赤色枠）
```
[エラー]
???????????????????????????????????
E003: 不正なCSV形式です
（ヘッダー行が不正）

詳細:
ヘッダー行の内容:
"PhysicalName,LogicalName,Value"
期待される内容:
"PhysicalName,LogicalName,Value,Unit,Description"

[OK]
???????????????????????????????????
```

#### 警告ダイアログ（黄色枠）
```
[警告]
???????????????????????????????????
W002: インポート処理に一部の問題がありました

結果:
- 5 行のデータがインポートされました
- 2 行はスキップされました
  (行3: PhysicalName が空です)
  (行7: PhysicalName の形式が不正です)

[OK] [詳細ログを表示]
???????????????????????????????????
```

#### 情報ダイアログ（青色枠）
```
[情報]
???????????????????????????????????
I002: ファイルに保存しました

ファイル: C:\Users\...\constants.csv
行数: 5 件
保存日時: 2026-01-21 14:30:45

[OK]
???????????????????????????????????
```

---

## 8. 非機能要件

### 8.1 パフォーマンス

- CSV インポート（1000行）: 1秒以内に完了
- グリッド表示: スクロール時のレスポンス 100ms以内
- エクスポート: 100行程度で 500ms以内

### 8.2 セキュリティ

- ファイルアクセスは Windowsのファイルシステムパーミッションに準ずる
- ユーザー入力値は XSS対策として無効化処理を施す
- パスワードなどの機密情報はValue に含めない（ユーザー責任）

### 8.3 ユーザビリティ

- 日本語UI対応
- キーボードショートカット対応（例：Ctrl+S で保存）
- メニューバーでの階層的なメニュー操作
- モーダルダイアログによる安全なデータ入力

### 8.4 保守性

- エラーログは `%APPDATA%\ConstantManager\logs\` に保存
- ログファイル: `YYYYMMDD.log`
- ログレベル: INFO, WARN, ERROR

---

## 9. 納品物チェックリスト

- [ ] 定数管理システム実行ファイル（.exe）
- [ ] インストーラー（.msi）
- [ ] ユーザーマニュアル（PDF）
- [ ] 技術仕様書（デザイン仕様書）
- [ ] テストレポート
- [ ] サンプルCSVファイル
- [ ] トラブルシューティングガイド

---

## 10. 改版履歴

| 版番 | 作成日 | 変更内容 |
|------|--------|---------|
| 1.0 | 2026-01-21 | 初版作成 |
| 1.1 | 2026-01-21 | ID→PhysicalName、Name→LogicalName に変更。PhysicalName は新規作成時のみ入力可、既存編集時は読み取り専用。アンドゥ・リドゥ機能を削除。DataGridView の列名を日本語に統一（定数名、日本語名、値、単位、説明）。 |
| 1.2 | 2026-01-21 | メイン画面上部に MenuStrip（メニューバー）を配置。画面下部の編集エリアと編集ボタンを削除。定数の追加・編集は「定数編集ダイアログ」で行うモーダル化に変更。ウィンドウ終了時の未保存変更確認機能を追加。ユースケースを新しいUI/UXに合わせて全面更新。 |

---

**文書管理情報**
- **作成者**: システム開発部
- **承認者**: プロジェクトマネージャー
- **最終更新**: 2026-01-21
- **ステータス**: 承認待ち
